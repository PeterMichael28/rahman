...
import homeIcon from "~/assets/images/footer/footer-icons/Home.webp";
import aboutIcon from "~/assets/images/footer/footer-icons/About.webp";
import resumeIcon from "~/assets/images/footer/footer-icons/Resume.webp";

import BgWeb from "~/assets/images/footer/footer-icons/footerBg.webp";
import BgMobile from "~/assets/images/footer/footer-icons/footerBgSm.webp";
    ...
    <>
      <footer className="relative bg-primary-light mt-24 min-h-[390px] lg:h-[432px]">
        ...
        <div className="absolute inset-0 w-full h-full md:hidden">
          <img src={BgMobile} alt="overlay" className=" w-full h-full" />
        </div>

        <div className="max-w-[1200px] w-full mx-auto px-5 md:px-4 pt-7 md:pt-[56px]">
          <div className="flex flex-col md:flex-row items-start md:justify-between md:items-center gap-y-4 relative z-10 w-full">
            <div>
              <p className="text-[#EAE3FC] text-[20px] md:text-[32px] md:leading-[32px] font-medium leading-[23px] tracking-[-0.36px]">
                Ready to build your product?
              </p>
              <h2 className="text-white text-[28px] md:text-[52px] md:leading-[57px] font-medium tracking-[-0.56px]  leading-[33px]">
                Lets work together.
              </h2>
            </div>

            <button className=" h-12 md:h-14 md:text-[18px] text-sm flex items-center justify-center rounded-lg transition-all duration-300 hover:text-primary text-primary-text font-medium btn-shadow w-[128px] md:w-[149px]">
              <a href="mailto:koleibrahimabdulrahman@gmail.com">Contact me</a>
            </button>
          </div>

          {/* divider */}
          <div className="w-full h-[1px] bg-white/5 rounded-md md:hidden my-8" />

          {/* contact details */}
          <div className="flex flex-col md:flex-row items-start md:justify-between w-full p-4 md:p-5  md:mt-[58px]  relative ">
            {/* contact overlays */}
            <div className="absolute inset-0 w-full h-full hidden md:block bg-[rgba(255,255,255,0.03)]  backdrop-blur-[50px] rounded-[16px]">
              {/* <img src={contactOverlay} alt="overlay" className="h-full w-full object-" /> */}
            </div>

            {/* contactOverlay2 small screen*/}
            <div className="absolute inset-0 w-full h-full md:hidden bg-[rgba(255,255,255,0.03)]  backdrop-blur-[50px] rounded-[16px]">
            ...
            <div className="flex justify-start md:justify-end gap-x-[96px] items-start relative z-20 w-full flex-col md:flex-row mt-8 md:mt-0">
              <nav className="space-y-4">
                <p className="text-[#eae3fc] font-medium text-sm">Quicklinks</p>
                <ul className="space-y-3 font-normal">
                  {navContacts.map((nav) => (
                    <li key={nav.label} className="text-white ">
                      <Link to={nav.href} className="flex items-center gap-2">
                        <img src={nav.img} alt={nav?.label + "icon"} className="size-4" />
                        <span className="text-base cmd:text-lg font-medium leading-[21px] md:leading-[24px]">
                          {nav.label}
                        </span>
                      </Link>
                    </li>
                  ...
                  <li className="cursor-pointer">
                    ...
                    >
                      <img src={resumeIcon} alt={"Resume icon"} className="size-4" />
                      <span className="md:text-lg font-medium leading-[21px] md:leading-[24px]">Resume</span>
                    </a>
                  </li>
                </ul>
              </nav>

              <nav className="space-y-4 mt-8 md:mt-0  max-md:w-full">
                <p className="text-[#eae3fc] font-medium text-sm ">Socials</p>
                <ul className="space-y-3 font-normal">
                  {socials.map((social) => (
                    <li key={social.title}>
                      <a
                        href={social.url}
                        target="_blank"
                        className="text-white flex items-center md:justify-between max-md:w-full"
                        rel="noreferrer"
                      >
                        <span className="flex items-center max-md:w-full">
                          <span className="text-[16px]">{social.icon}</span>
                          <span className="ml-2 text-base font-medium leading-[21px]">{social.title}</span>
                        </span>
                        <span className="md:ml-3 ">{icons.arrowUpRight}</span>
                      </a>
                    </li>
                  ))}
                </ul>
            ...
            <p className="text-white text-xs md:hidden relative mt-10 z-20">
              &copy; ABDULRAHMAN KOLE-IBRAHIM {new Date().getFullYear()}
            </p>
          </div>

          {/* mobile footer tag */}
          <div className="mt-[2px] relative h-12 md:hidden">
            <div className="absolute inset-0 w-full h-full  bg-[rgba(255,255,255,0.03)]  backdrop-blur-[50px] rounded-t-[16px] ">
              <div className="w-full h-full text-xs flex justify-center items-center gap-x-1 gap-y-1 relative text-white text-center playfair-display-footer">
                <p className=" leading-4 italic font-normal">
                  Designed by <span className="">Oluwatimilehin Akapo.</span>
                </p>

                <a href="mailto:akapooluwatimilehin@gmail.com" className="italic font-semibold">
                  Contact me
                </a>
              </div>
            </div>
          ...
          <div className="mt-[2px] relative h-14 md:block hidden ">
            <div className="absolute inset-0 w-full h-full  bg-[rgba(255,255,255,0.03)]  backdrop-blur-[50px] rounded-t-[16px] ">
              <div className="flex justify-between items-center w-full h-full px-5">
                <p className="text-white text-base leading-[21px] font-normal">
                  &copy; ABDULRAHMAN KOLE-IBRAHIM {new Date().getFullYear()}
                </p>

                <div className=" text-base flex justify-center items-center gap-x-1 gap-y-1 relative text-white text-center playfair-display-footer">
                  <p className=" leading-[21px] italic font-normal">
                    Designed by <span className="">Oluwatimilehin Akapo.</span>
                  </p>
